...
impl ChunkPos {
    pub fn from_world(x: i32, z: i32) -> Self {
        Self(x.div_euclid(CHUNK_SIZE as i32), z.div_euclid(CHUNK_SIZE as i32))
    }
...
impl BlockSection {
    ...
    pub fn set(&mut self, x: usize, y: usize, z: usize, id: u8) {
        ...
        if prev != 0 && id == 0 { self.non_air = self.non_air.saturating_sub(1); }
        else if prev == 0 && id != 0 { self.non_air += 1; }
        self.blocks[i] = id;
    }
...
impl Chunk {
    pub fn new(pos: ChunkPos) -> Self {
        Self { pos, sections: std::array::from_fn(|_| None), generated: false, dirty: false, mesh_dirty: true }
    }
    pub fn get_block(&self, x: usize, y: usize, z: usize) -> u8 {
        if y >= CHUNK_HEIGHT { return 0; }
        let sec = y / SECTION_SIZE;
    ...
    pub fn set_block(&mut self, x: usize, y: usize, z: usize, id: u8) {
        ...
        let sec = y / SECTION_SIZE;
        let ly = y % SECTION_SIZE;
        if self.sections[sec].is_none() {
            if id == 0 { return; }
            self.sections[sec] = Some(BlockSection::empty());
        }
