...
/**
 * Screenshot generator driven by `:games:logicgate:metadata`.
 * Captures progression list and a level's circuit canvas.
 */
...
class MetadataScreenshots {
    ...
    fun generateStoreScreenshots() {
        composeRule.waitUntil(timeoutMillis = 15_000) {
            composeRule.onAllNodesWithText("Logic", substring = true).fetchSemanticsNodes().isNotEmpty() ||
                composeRule.onAllNodesWithText("Level", substring = true).fetchSemanticsNodes().isNotEmpty()
        }
        Thread.sleep(1000)
        snap(1)
        snap(2)
