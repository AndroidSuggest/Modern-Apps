...
/**
 * Screenshot generator driven by `:vpn:metadata`.
 * Captures config list / empty state.
 */
...
class MetadataScreenshots {
    ...
    fun generateStoreScreenshots() {
        composeRule.waitUntil(timeoutMillis = 15_000) {
            composeRule.onAllNodesWithText("VPN", substring = true).fetchSemanticsNodes().isNotEmpty() ||
                composeRule.onAllNodesWithText("Config", substring = true).fetchSemanticsNodes().isNotEmpty()
        }
        Thread.sleep(1000)
        snap(1)
        snap(2)
