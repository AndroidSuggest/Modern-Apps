...
| `maps` ratings + live traffic | **Google Places + traffic via self-hosted proxy (transitive)** | `https://api.vayunmathur.com/maps/traffic` (live traffic tiles, `OfflineRouter.kt`), `place_match`, `place_rating` wrapping Google Places (`Reviews.kt`) | Ratings/traffic proxy holds secret; Google is upstream. **Routing is fully offline** via `offlinerouter.so` (C++ `maps/src/main/cpp/`, `OfflineRouter.findRouteNative`) + self-hosted `v4.pmtiles` + zones `zone_*.pmtiles` on `data.vayunmathur.com` + `admin0.fgb/admin1.fgb` + GTFS — no online routing endpoint is called | Place ratings + live-traffic overlay fail; turn-by-turn routing continues fully offline with pre-downloaded zones | Ratings could be OSM/Wikidata; self-host a traffic source. (Online routing already removed in favor of the offline router) |
| `maps` basemap (vector) | **Protomaps v4 + basemaps-assets now self-hosted ✅** | `https://data.vayunmathur.com/v4.pmtiles` (137 GB, was `https://demo-bucket.protomaps.com/v4.pmtiles`), zones `https://data.vayunmathur.com/zone_*.pmtiles` (64 via Morton/Z-order, `ZoneDownloadManager.kt:170`), styling sprites `https://data.vayunmathur.com/basemaps-assets/sprites/v4/light` + fonts `fonts/{fontstack}/{range}.pbf` (`style.json:3541-3542`, 1044 objects), offline routing `amenities.db`, `metadata.bin`, `road_names.bin` | Self-hosted on R2 bucket `maps` custom domain `data.vayunmathur.com` (Cloudflare, direct Range, free egress, `immutable` cache). PMTiles streams via HTTP Range by MapLibre; disk cache `MapTileCache.kt:36-39` SHA-256(URL+Range) 24h refresh + offline stale-serve with `.origin` migration marker. Attribution `Protomaps © OSM` preserved (`style.json` attribution, ODbL/BSD allowed) | If R2 down: previously-cached tilecache still serves Ultra; zone offline still works; fallback would require demo-bucket again (comment kept in vendor script) | Already vendored at `data.vayunmathur.com` — zero external vector dependency. APK only hits own domain for vector tiles |
...
- `third_party/jni-android` fork: Removed build-dependencies `walkdir 2` (which pulled `same-file 1.0.6 -> winapi-util 0.1.11 -> windows-sys 0.61.2 -> windows-link + windows-targets 0.42.2 + 7 arch shims = 11 crates per lock). Also removed `thiserror-impl` unused? Kept `cesu8`, `combine`, `jni-sys` (required for Modified UTF-8). Also stripped optional `invocation` (java-locator, libloading pulled glob, windows, lazy_static, rusty-fork, tempfile, wait-timeout). Result: root lock 0 walkdir/0 same-file/0 winapi-util/0 windows-sys 0.45+0.61.
...
- `games/voxels`: Regenerated its own workspace lock (outside root workspace) after patching jni path via `../../../../../third_party/jni-android`. Dropped walkdir/windows – from 60 → 47 packages (no walkdir/windows-sys).
