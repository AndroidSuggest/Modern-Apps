...
| `youpipe` extras | **SponsorBlock / DeArrow (community)** | `https://sponsor.ajay.app/api/skipSegments?videoID=`, `/api/branding?videoID=`, `https://dearrow-thumb.ajay.app/api/v1/getThumbnail?videoID=` | Crowdsourced, best-effort extras, not core YouTube | Sponsor skips + DeArrow thumbnails/branding missing, degrades gracefully | Self-host SponsorBlock mirror, or disable feature flag |
