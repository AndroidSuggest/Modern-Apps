//! Reduction over one or more axes: `ReduceMean`, `ReduceSum`, `ReduceMax`,
//! `ReduceMin`.
//!
...
//! the graph in 18 places. Multi-axis reductions (Selfie's `ReduceMean`,
//! Supertonic's `ReduceSum`) compose one per-axis pass at a time. Each test
//! compares the kernel against a CPU reference written here.
...
/// CPU reference over a **set** of axes, output in row-major order of the
/// non-reduced dimensions (identical whether or not `keepdims` is set).
fn reduce_multi_ref(x: &[f32], shape: &[i64], axes: &[usize], kind: &str) -> Vec<f32> {
    let rank = shape.len();
    let dims: Vec<usize> = shape.iter().map(|&d| d as usize).collect();
    let reduced: Vec<bool> = (0..rank).map(|d| axes.contains(&d)).collect();
    let out_len: usize = (0..rank)
        .filter(|&d| !reduced[d])
        .map(|d| dims[d])
        .product::<usize>()
        .max(1);
    let reduced_count: usize = (0..rank).filter(|&d| reduced[d]).map(|d| dims[d]).product();
    let mut acc = match kind {
        "max" => vec![f32::NEG_INFINITY; out_len],
        "min" => vec![f32::INFINITY; out_len],
        _ => vec![0.0f32; out_len],
    };
    let total: usize = dims.iter().product::<usize>();
    for lin in 0..total {
        let mut rem = lin;
        ...
        for d in 0..rank {
            if !reduced[d] {
                oidx = oidx * dims[d] + coord[d];
            }
        }
        let v = x[lin];
        match kind {
            "max" => acc[oidx] = acc[oidx].max(v),
            "min" => acc[oidx] = acc[oidx].min(v),
            _ => acc[oidx] += v,
        }
    }
    if kind == "mean" {
...
/// Selfie's shape: `ReduceMean` over two axes with `keepdims = 1`.
#[test]
fn reduce_mean_multi_axis_keepdims() {
    let shape = vec![2i64, 3, 4, 5];
    let x = pseudo(120, 41);
    let ir = graph(
        vec![node(
            "ReduceMean",
    ...
    assert_eq!(out_shape, vec![2, 1, 4, 1]);
    assert_close(&got, &reduce_multi_ref(&x, &shape, &[1, 3], "mean"));
}

/// `ReduceSum` over two axes with `keepdims = 0` — the dimensions are removed,
/// and the highest-first order keeps the lower index valid as rank drops.
#[test]
fn reduce_sum_multi_axis_no_keepdims() {
    let shape = vec![2i64, 3, 4, 5];
    let x = pseudo(120, 43);
    let ir = graph(
        vec![node(
    ...
    let (got, out_shape) = run(&ir, HostTensor::from_f32(shape.clone(), &x), "out");
    assert_eq!(out_shape, vec![3, 5]);
    assert_close(&got, &reduce_multi_ref(&x, &shape, &[0, 2], "sum"));
}

/// `ReduceMin` over two axes, with a negative axis to exercise normalization.
#[test]
fn reduce_min_multi_axis_negative() {
    let shape = vec![2i64, 3, 4, 5];
    let x = pseudo(120, 47);
    let ir = graph(
        vec![node(
    ...
    let (got, out_shape) = run(&ir, HostTensor::from_f32(shape.clone(), &x), "out");
    assert_eq!(out_shape, vec![2, 1, 4, 1]);
    assert_close(&got, &reduce_multi_ref(&x, &shape, &[1, 3], "min"));
}

/// `ReduceMin` with axes in a constant input is folded like the others: the
/// fourth entry of `AXES_AS_INPUT` (added for opset 18).
#[test]
fn reduce_min_constant_axes_input_is_folded() {
    ...
    let mut n = node("ReduceMin", &["x", "axes"], &["out"], &[]);
    ...
    assert_eq!(n.inputs, vec!["x".to_string()]);
...
/// What the kernel **cannot** do must not be claimed: axes as an unresolved
/// input, and missing `axes`. Claiming them would split the block at runtime
/// instead of during capability resolution. Multiple axes, by contrast, are
/// now claimed and composed one axis at a time.
#[test]
fn unsupported_forms_are_not_claimed() {
    ...
    assert!(
        ...
        "multiple axes are now implemented"
    );
