...
/// `ceil_mode = 1`: the output extent rounds up and the extra trailing windows
/// pool only the in-bounds elements (the kernel clamps the window). This is the
/// form U2Net's 33 `MaxPool` nodes use.
#[test]
fn maxpool_ceil_mode_rounds_up_and_clamps_windows() {
    let (c, h, w) = (2usize, 5usize, 5usize);
    let x = pseudo(c * h * w, 71);
    let ir = graph(
        vec![node(
            "MaxPool",
            &["x"],
    ...
    // ceil((5 - 2) / 2) + 1 = 3 on each spatial axis (floor would give 2)
    let (h_out, w_out) = (3usize, 3usize);
    let mut want = vec![f32::NEG_INFINITY; c * h_out * w_out];
    for ch in 0..c {
        for oh in 0..h_out {
            for ow in 0..w_out {
                ...
                for r in 0..2 {
                    ...
                    for s in 0..2 {
                        ...
                        acc = acc.max(x[(ch * h + ih) * w + iw]);
                    }
                }
                want[(ch * h_out + oh) * w_out + ow] = acc;
            }
        }
    }
    assert_eq!(got.len(), c * h_out * w_out, "ceil rounds the extent up");
    assert_close(&got, &want);
}
