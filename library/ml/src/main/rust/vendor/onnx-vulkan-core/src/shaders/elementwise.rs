...
mod tests {
    ...
    #[test]
    fn added_activation_sources_compile() {
        // HardSwish, Softplus, HardSigmoid ride the UNARY template.
        compile_wgsl(&UNARY.replace("OP", "v * clamp(v * 0.16666667 + 0.5, 0.0, 1.0)"))
            .expect("HardSwish unary valido");
        compile_wgsl(&UNARY.replace("OP", "max(v, 0.0) + log(1.0 + exp(-abs(v)))"))
            .expect("Softplus unary valido");
        compile_wgsl(&UNARY.replace("OP", "clamp(0.2 * v + 0.5, 0.0, 1.0)"))
            .expect("HardSigmoid unary valido");
        // PRelu rides the broadcasting BINARY template.
        compile_wgsl(&BINARY.replace("OP", "select(a[off_a] * b[off_b], a[off_a], a[off_a] >= 0.0)"))
            .expect("PRelu binary valido");
    }
}
