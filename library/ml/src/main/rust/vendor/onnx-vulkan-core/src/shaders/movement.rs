...
/// `Pad`; output shape, initial pads, shape and strides of input arrive in an
/// i32 `params` buffer. `mode` selects the border behaviour: 0 = constant (out
/// of range reads `cval`), 1 = edge (clamp to the nearest valid index), 2 =
/// reflect (mirror across the border without repeating the edge).
pub const PAD: &str = r#"
@group(0) @binding(0) var<storage, read> src: array<f32>;
@group(0) @binding(1) var<storage, read> params: array<i32>;
@group(0) @binding(2) var<storage, read_write> dst: array<f32>;
struct Push { n: u32, rank: u32, cval: f32, mode: u32 }
var<immediate> pc: Push;
@compute @workgroup_size(256)
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    let o = gid.x;
    ...
    for (var d = 0u; d < pc.rank; d = d + 1u) {
        ...
        let istride = params[1u + 3u * pc.rank + d];
        var sc = ic;
        if (pc.mode == 0u) {
            if (ic < 0 || ic >= idim) { oob = true; }
        } else if (pc.mode == 1u) {
            sc = clamp(ic, 0, idim - 1);
        } else {
            if (idim <= 1) {
                sc = 0;
            } else {
                let period = 2 * (idim - 1);
                var m = ((ic % period) + period) % period;
                if (m >= idim) { m = period - m; }
                sc = m;
            }
        }
        si = si + sc * istride;
...
pub const PAD_PUSH_BYTES: u32 = 16;
