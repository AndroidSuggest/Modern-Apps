...
/// Converts a single `GraphProto` — the model's main graph or a subgraph
/// carried by a `Graph` attribute (`If`'s `then_branch`/`else_branch`, a
/// `Loop`/`Scan` body) — into the core's IR.
///
/// Subgraphs carry no opset imports of their own; they inherit the enclosing
/// model's, so `opsets` is threaded down from [`model_to_ir`]. Subgraph-local
/// initializers and `Constant` outputs are captured here exactly as for the
/// main graph. Values the subgraph reads from the outer scope stay as inputs;
/// the core merges that scope when it inlines the branch.
fn graph_to_ir(
...
fn attribute_to_ir(
    ...
    Ok(Some(match kind {
        ...
        // A single subgraph: `If`'s branches and the `Loop`/`Scan` bodies.
        // Converted with the very same machinery as the main graph so a nested
        // node is read identically wherever it sits, and boxed because the IR
        // is recursive (`GraphIr` → `NodeIr` → `AttrValue::Graph`).
        AttributeType::Graph => {
            let graph = attribute
                .g
                .as_ref()
        ...
        // the list-of-graphs form (`GRAPHS`), multiple strings, sparse tensors
        // and type-proto have no place in the core's IR yet, so the node
        // carrying them must be rejected outright, not silently emptied
