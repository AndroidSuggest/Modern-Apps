...
impl Ctx<'_> {
    ...
    #[cfg(test)]
    fn for_tests(node_attrs: &NodeIr) -> Ctx<'static> {
        // the legacy Slice tests below carry no constant inputs: attributes
        // are the only parameter source, so empty maps suffice.
        let _ = node_attrs;
        Ctx {
            types: Box::leak(Box::new(HashMap::new())),
            constants: Box::leak(Box::new(HashMap::new())),
        }
    }
}
...
///
/// Reads the pre-opset-10 attribute form too (`starts`/`ends`/`axes`
/// attributes, no `steps`): the runtime `slice_param` prefers attributes over
/// inputs, and inference mirrors that precedence so the planned shape can
/// never disagree with the executed one. A legacy node with neither form
/// still yields `None` (unknown), never a guess.
fn slice(node: &NodeIr, ctx: &Ctx, input: &Option<Vec<Dim>>) -> Option<Vec<Dim>> {
    let dims = input.as_ref()?;
    let starts = attr_ints(node, "starts").or_else(|| ctx.constant_ints(node, 1))?;
    let ends = attr_ints(node, "ends").or_else(|| ctx.constant_ints(node, 2))?;
    let axes = match attr_ints(node, "axes").or_else(|| ctx.constant_ints(node, 3)) {
        Some(axes) => axes,
        None => (0..starts.len() as i64).collect(),
    };
    let steps = attr_ints(node, "steps")
        .or_else(|| ctx.constant_ints(node, 4))
...
#[cfg(test)]
mod tests {
    use super::{Ctx, Dim, TensorType, attr_ints, axis, ceil_div, infer, slice};
    use onnx_vulkan_core::host_ops::FLOAT;
    use onnx_vulkan_core::{AttrValue, GraphIr, NodeIr};
    use std::collections::HashMap;

    /// A pre-opset-10 `Slice` node: starts/ends/axes as attributes, one data
    /// input, the PP-OCR-rec form.
    fn legacy_slice_node(starts: Vec<i64>, ends: Vec<i64>, axes: Option<Vec<i64>>) -> NodeIr {
        let mut attrs = HashMap::from([
            ("starts".to_string(), AttrValue::Ints(starts)),
            ("ends".to_string(), AttrValue::Ints(ends)),
        ]);
        if let Some(axes) = axes {
            attrs.insert("axes".to_string(), AttrValue::Ints(axes));
        }
        NodeIr {
            domain: String::new(),
            op: "Slice".to_string(),
    ...
    fn fixed(shape: Vec<i64>) -> Option<Vec<Dim>> {
        Some(shape.into_iter().map(Dim::Fixed).collect())
    }

    /// The legacy attribute form infers the same shape as the input form:
    /// negative bounds count from the end, omitted axes default to
    /// `[0..len)`, `ends` is exclusive.
    #[test]
    fn legacy_attribute_form_infers_exact_shapes() {
        let node = legacy_slice_node(vec![1, -3], vec![3, -1], None);
        let input = fixed(vec![3, 4]);
        let ctx = Ctx::for_tests(&node);

        assert_eq!(slice(&node, &ctx, &input), fixed(vec![2, 2]));
    }

    /// `ends = INT64_MAX` ("to the end", as legacy exporters write it)
    /// saturates to the dimension.
    #[test]
    fn legacy_int64_max_end_saturates() {
        let node = legacy_slice_node(vec![2], vec![i64::MAX], Some(vec![0]));
        let input = fixed(vec![5]);
        let ctx = Ctx::for_tests(&node);

        assert_eq!(slice(&node, &ctx, &input), fixed(vec![3]));
    }

    /// Explicit axes (including negative) are honored in the attribute form.
    #[test]
    fn legacy_explicit_axes_are_honored() {
        let node = legacy_slice_node(vec![1], vec![3], Some(vec![-1]));
        let input = fixed(vec![4, 5]);
        let ctx = Ctx::for_tests(&node);

        assert_eq!(slice(&node, &ctx, &input), fixed(vec![4, 2]));
    }

    /// End to end: a one-node legacy Slice graph infers the output shape
    /// through the public entry point, with no constants involved.
    #[test]
    fn legacy_slice_graph_infers_output_shape() {
        let node = legacy_slice_node(vec![0, 1], vec![2, 4], None);
        let ir = GraphIr {
            nodes: vec![node],
            inputs: vec!["x".to_string()],
            outputs: vec!["y".to_string()],
        ...
        let declared = HashMap::from([(
            "x".to_string(),
            TensorType::of(FLOAT, vec![Dim::Fixed(2), Dim::Fixed(4)]),
        )]);

        let (types, conflicts) = infer(&ir, &declared);

        assert!(conflicts.is_empty(), "unexpected conflicts: {conflicts:?}");
        assert_eq!(
            types.get("y").and_then(|t| t.concrete()),
            Some(vec![2, 3]),
            "y = x[0:2, 1:4]"
        );
    }
    ...
    fn helpers_behave() {
        ...
        let node = legacy_slice_node(vec![1], vec![2], None);
        assert_eq!(attr_ints(&node, "starts"), Some(vec![1]));
        assert_eq!(attr_ints(&node, "missing"), None);
    }
}
