...
use super::paint::{parse, parse_hex, Ramp, Stroke, FLAT, MIN_HALF_WIDTH_PX};
use super::paint_extra2::{authored_layer, basemap, dash_drawn, find, line_layers, ZOOMS};
...
fn the_vendored_flat_style_parses() {
    ...
    assert_eq!(
        (
            count(LayerKind::Fill, true),
            count(LayerKind::Line, true),
            count(LayerKind::Symbol, true)
        ),
        (0, 1, 6),
...
fn the_poi_colour_groups_partition_the_kinds_the_reference_admits() {
    ...
    for clause in filter {
        let Some(clause) = clause.as_array() else {
            continue;
        };
        if clause.first().and_then(Json::as_str) != Some("in") {
            continue;
        }
        let Some(literal) = clause.get(2).and_then(Json::as_array) else {
            continue;
        };
        let Some(names) = literal.get(1).and_then(Json::as_array) else {
            continue;
        };
    ...
    let (local, reference): (Vec<String>, Vec<String>) = drawn
        .into_iter()
        .partition(|kind| LOCAL.contains(&kind.as_str()));
    assert_eq!(
        local, LOCAL,
        "the local POI extension is exactly these four kinds"
    );
    admitted.sort_unstable();
    assert_eq!(
        reference, admitted,
        "the POI layers do not cover the reference filter"
    );
}
...
fn a_malformed_style_fails_to_load_rather_than_loading_partly() {
    let cases = [
        (r##"{}"##, "no background"),
        (
            r##"{"background":{"light":"#fff","dark":"#000"},"layers":[]}"##,
            "a bad colour",
        ),
        (
...
fn a_ramp_picks_the_right_segment_of_several() {
    let ramp = Ramp {
        base: 1.0,
        stops: vec![(0.0, 0.0), (10.0, 10.0), (20.0, 0.0)],
    };
...
fn every_line_layer_draws_something_somewhere_in_the_zoom_range() {
    for layer in line_layers() {
        let drawn = ZOOMS
            .into_iter()
            .any(|zoom| layer.stroke(zoom as f64).visible());
...
fn a_highway_at_world_zoom_is_a_small_fraction_of_its_width_at_street_zoom() {
    ...
    assert!(
        world > 0.0 && street > 0.0,
        "world {world}, street {street}"
    );
    assert!(
        world < street * 0.15,
...
fn width_grows_continuously_with_zoom_rather_than_stepping_per_level() {
    ...
    let highway = find("roads-highway");
    let widths: Vec<f32> = (0..=10)
        .map(|step| highway.stroke(13.5 + step as f64 / 10.0).width_dp)
        .collect();
    for pair in widths.windows(2) {
        assert!(
            pair[1] > pair[0],
            "width stepped rather than grew: {widths:?}"
        );
    }
...
fn an_ungated_road_layer_is_still_not_drawn_at_world_zoom() {
    ...
    assert_eq!(
        highway.min_zoom, 0,
        "this asserts the ramp gates, not `min_zoom`"
    );
    for tenth in 0..=30 {
        let zoom = tenth as f64 / 10.0;
...
fn a_layer_is_gapped_exactly_when_its_gap_ramp_is_ever_non_zero() {
    for layer in layers() {
        ...
        assert_eq!(
            layer.gapped(),
            ever,
            "`{}` disagrees with its gap ramp",
            layer.id
        );
    }
    assert!(find("roads-major-casing").gapped());
    assert!(
        find("roads-link-casing").gapped(),
        "a link casing is still a casing"
    );
    assert!(!find("roads-major").gapped());
    assert!(
        !find("roads-link").gapped(),
        "a link fill is a single centred stroke"
    );
