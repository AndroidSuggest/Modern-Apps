...
fn the_pixel_widths_at_the_measured_zooms_are_what_the_style_asks_for() {
    ...
    assert!(
        minor < highway,
        "a minor road must be narrower than a highway"
    );
    ...
    assert!(
        link < highway,
        "a slip road must be narrower than the highway it joins"
    );
}
...
fn a_link_is_narrower_than_the_class_road_it_joins() {
    ...
    for tenth in [160, 170, 180] {
        ...
        assert!(
            l < h,
            "link {l} Dp is not narrower than highway {h} Dp at z{zoom}"
        );
    }
    assert_eq!(
        link.stroke(18.0).width_dp,
        11.0,
        "the authored z18 link width"
    );
    ...
    assert!(
        highway.forbid_flags & 0b100 != 0,
        "highway layers forbid links"
    );
}
...
fn a_service_street_is_narrower_than_a_minor_road() {
    ...
    assert_eq!(
        service.stroke(18.0).width_dp,
        8.0,
        "the authored z18 service width"
    );
...
fn fill_opacity_is_a_ramp_evaluated_per_frame_not_a_baked_alpha() {
    ...
    for id in [
        ...
        assert_eq!(
            find(id).light >> 24,
            0xFF,
            "{id}'s alpha belongs to its ramp"
        );
    ...
    assert_eq!(
        at("landcover:grassland", 6.0),
        0.5,
        "an arm carries its family's ramp"
    );
    assert_eq!(at("landuse_park:national_park", 6.0), 0.0);
    assert!(
        (at("landuse_park:wood", 7.0) - 0.2).abs() < 1e-6,
        "a fifth, not full green"
    );
    ...
    for layer in layers() {
        match layer.kind {
            ...
            LayerKind::Symbol => {
                ...
                assert!(
                    layer.halo_light >> 24 > 0,
                    "{} halo is fully transparent",
                    layer.id
                );
            }
        }
...
fn city_labels_track_the_big_city_arm_at_compared_zooms() {
    ...
    assert_eq!(
        size("places-locality", 10.0, 13),
        20.0,
        "a million-plus city"
    );
    assert_eq!(
        size("places-locality", 10.0, 9),
        20.0,
        "50k is on the threshold"
    );
