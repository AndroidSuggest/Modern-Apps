...
fn points_one_tile_unit_apart_still_give_a_unit_normal() {
    ...
    assert!(
        (normal_len(&v, 0) - 1.0).abs() < 1e-5,
        "start {}",
        normal_len(&v, 0)
    );
    let last = v.len() / FLOATS_PER_VERTEX - 1;
    assert!(
        (normal_len(&v, last) - 1.0).abs() < 1e-5,
        "end {}",
        normal_len(&v, last)
    );
}
...
fn a_tapered_end_is_exactly_the_ratio_and_the_far_end_is_untouched() {
    ...
    ribbon_tapered(
        &[0, 0, 1000, 0],
        1000,
        Taper::new(0.5, 1.0, 0.2),
        &mut v,
        &mut Vec::new(),
    );
    let last = v.len() / FLOATS_PER_VERTEX - 1;
    assert!(
        (kerb_offset(&v, 0, 40.0) - 20.0).abs() < 1e-5,
        "start is half width"
    );
    assert!(
        (kerb_offset(&v, last, 40.0) - 40.0).abs() < 1e-5,
        "end is full width"
    );
}
...
fn the_ramp_reaches_full_width_at_the_run_and_stays_there() {
    ...
    assert!(
        (position_x(&v, 2) - 0.2).abs() < 1e-5,
        "at {}",
        position_x(&v, 2)
    );
    assert!(
        (kerb_offset(&v, 2, 40.0) - 40.0).abs() < 1e-5,
        "full width from the run on"
    );
}
...
fn a_taper_narrows_monotonically_and_never_inverts_the_road() {
    ...
    let widths: Vec<f32> = (0..n)
        .step_by(2)
        .map(|i| kerb_offset(&v, i, 40.0))
        .collect();
    assert!(
        widths.iter().all(|w| *w > 0.0),
        "a zero width draws nothing: {widths:?}"
    );
    assert!(
        widths.iter().all(|w| *w <= 40.0 + 1e-5),
        "never wider than the push: {widths:?}"
    );
    assert!(
        (widths[0] - 10.0).abs() < 1e-5,
        "start quarter width: {}",
        widths[0]
    );
    let end = widths.len() - 1;
    assert!(
        (widths[end] - 20.0).abs() < 1e-5,
        "end half width: {}",
        widths[end]
    );
    ...
    assert!(
        (peak - 40.0).abs() < 1e-5,
        "the middle reaches full width: {widths:?}"
    );
}
...
fn a_ramp_that_lands_on_an_existing_vertex_inserts_nothing() {
    ...
    assert_eq!(
        v.len() / FLOATS_PER_VERTEX,
        6,
        "three points, no fourth inserted"
    );
    assert!(
        (kerb_offset(&v, 2, 40.0) - 40.0).abs() < 1e-5,
        "and it is the full-width point"
    );
}
...
fn a_ratio_outside_the_unit_range_cannot_be_constructed() {
    ...
    assert_eq!(
        Taper::new(f32::NAN, f32::INFINITY, 0.1),
        Taper::new(1.0, 1.0, 0.1)
    );
    assert!(
        Taper::new(0.5, 0.5, -1.0).is_flat(),
        "a negative run cannot ramp"
    );
