...
object WhatsAppClient {
    ...
    private val _events = MutableSharedFlow<WhatsAppEvent>(extraBufferCapacity = 256)
    ...
    private var webSocket: WhatsAppSocket? = null
     ...
     */
    suspend fun sendCallOffer(conversationId: String, video: Boolean = false): Boolean {
        ...
        val node = WhatsAppCallSignaling.buildOffer(to, callId, auth.wid, video, encs, stanzaId)
        val sent = ws.send(WhatsAppProtocol.encodeNode(node))
    ...
    suspend fun rejectCall(from: String, callId: String, callCreator: String): Boolean {
        val ws = webSocket ?: return false
        return ws.send(
            WhatsAppProtocol.encodeNode(
                WhatsAppCallSignaling.buildReject(from, callId, callCreator, generateMessageId()),
            ),
        )
    }
