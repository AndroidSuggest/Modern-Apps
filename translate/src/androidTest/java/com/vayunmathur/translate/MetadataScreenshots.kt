...
/**
 * Screenshot generator driven by `:translate:metadata`.
 * Models (~1.2 GB) aren't bundled; captures download gate UI.
 */
...
class MetadataScreenshots {
    ...
    fun generateStoreScreenshots() {
        composeRule.waitUntil(timeoutMillis = 15_000) {
            composeRule.onAllNodesWithText("Translate", substring = true).fetchSemanticsNodes().isNotEmpty() ||
                composeRule.onAllNodesWithText("Download", substring = true).fetchSemanticsNodes().isNotEmpty()
        }
        Thread.sleep(1000)
        snap(1)
        snap(2)
